...
:: 	Now if we could I would test archive - BUT I CANT COZ WINMOUNT CANT DO THAT SO I JUST HAVE TO DELETE ALL SOURCE FOLDERS
::  note I've specified recursive testing of folders inside the archive,
...
echo.>>%log%
echo.>>%log%
echo.ARCHIVE OF %folder% MIGHT OR MIGHT NOT BE OK - DELETING SOURCE ANYWAY | "%scriptdir%/mtee.exe" /+ %log%
rd /s /q %folder% 2>&1
set error=%errorlevel%
if %error% EQU 0 echo.%shortfolder% DELETED from %folderdir% - Who knows if it worked you might have lost the source! | "%scriptdir%/mtee.exe" /+ %log%
