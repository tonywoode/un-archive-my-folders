...
echo.Datestamp is %date%, %time% ^& Current directory is %cd% >> 7ZIP_MY_FOLDERS.log
...
echo.>>7ZIP_MY_FOLDERS.log
echo.>>7ZIP_MY_FOLDERS.log
echo.ARCHIVE FOLDER CHECKS OK - DELETING SOURCE | "%scriptdir%/mtee.exe" /+ 7ZIP_MY_FOLDERS.log
rd /s /q "%folder%" 2>&1
set error=%errorlevel%
if %error% EQU 0 echo.%shortfolder% DELETED from %folderdir% AS 7z TESTED OK | "%scriptdir%/mtee.exe" /+ 7ZIP_MY_FOLDERS.log
if %error% NEQ 0 echo.ERROR removing folder %shortfolder% from %folderdir% | "%scriptdir%/mtee.exe" /+ 7ZIP_MY_FOLDERS.log
...
echo.| "%scriptdir%/mtee.exe" /+ 7ZIP_MY_FOLDERS.log
echo.| "%scriptdir%/mtee.exe" /+ 7ZIP_MY_FOLDERS.log
echo.REMOVED FOLDERS ARE THESE:| "%scriptdir%/mtee.exe" /+ 7ZIP_MY_FOLDERS.log
echo.(PRESS SPACE FOR NEXT PAGE)
echo.| "%scriptdir%/mtee.exe" /+ 7ZIP_MY_FOLDERS.log
echo.| "%scriptdir%/mtee.exe" /+ 7ZIP_MY_FOLDERS.log
find "DELETED"<7ZIP_MY_FOLDERS.log | "%scriptdir%/mtee.exe" /+ 7ZIP_MY_FOLDERS.log |more
echo.
