...
function s_doPlugins(s) {
	...
	/* Plugin: getQueryParam v2.1 */
	if(!s.campaign)
		s.campaign=s.getQueryParam('cid,code',":"); /*add all query string parameters here comma delimited with cid*/      
	...
	s.prop17=s.getTimeParting('h','-5',currentYear); // Set Hour
	s.eVar17=s.getValOnce(s.prop17,'s_evar17',0);
	s.prop18=s.getTimeParting('d','-5',currentYear); // Set Day
	...
	s.prop21=s.getPreviousValue(s.pageName,'gpv_p5','');

	/* Plugin Example: getDaysSinceLastVisit v1.1 */
	s.prop22=s.getDaysSinceLastVisit('s_lv');
	s.eVar22=s.getValOnce(s.prop22,'s_evar22',0);
	s.prop27=s.eVar22;
 ...
 *         If you want to capture the previous value on every page, 
