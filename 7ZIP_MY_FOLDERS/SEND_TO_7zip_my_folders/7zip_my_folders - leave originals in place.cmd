...
::	get the script directory for mtee - if we don't do this now shift will lose it. Also set directory to folders
...
set log="%folderdir%7ZIP_MY_FOLDERS_%RANDOM%.log"
...
::	get folder name, set a short filename for the error log too - filename with extension
...
echo.Made 7Z Archive %shortfolder%.7z now testing archive... | "%scriptdir%/mtee.exe" /+ %log%
"C:\Program Files\7-Zip\7z.exe" t -r %folder%.7z >>%log% 2>&1
set error=%errorlevel%
if %error% EQU 0 echo.%shortfolder%.7z CREATED AND TESTED - all OK | "%scriptdir%/mtee.exe" /+ %log%
...
if exist %shortfolder%.7z echo.7zip also already exists for empty folder %shortfolder%  | "%scriptdir%/mtee.exe" /+ %log%
if exist %shortfolder%.7z goto 7ZEXISTS
goto CONTINUE

:7ZEXISTS
echo.Datestamp is %date%, %time% ^& Current directory is %cd% >> %log%
echo.%shortfolder%.7z already exists - checking... | "%scriptdir%/mtee.exe" /+ %log%
"C:\Program Files\7-Zip\7z.exe" t %folder%.7z >>%log% 2>&1
set error=%errorlevel%
if %error% EQU 0 echo.%shortfolder%.7z ALREADY EXISTS and appears valid ERROR code is %error% | "%scriptdir%/mtee.exe" /+ %log%
if %error% NEQ 0 echo.%shortfolder%.7z ALREADY EXISTS and APPEARS CORRUPT ERROR code is %error% | "%scriptdir%/mtee.exe" /+ %log%
goto CONTINUE
...
:7ZIPALREADY
echo.Datestamp is %date%, %time% ^& Current directory is %cd% >> %log%
echo.%shortfolder% is a 7zip file already - checking... | "%scriptdir%/mtee.exe" /+ %log%
"C:\Program Files\7-Zip\7z.exe" t %folder% >>%log% 2>&1
set error=%errorlevel%
if %error% EQU 0 echo.%shortfolder% is ALREADY a 7Zip and appears ok ERROR code is %error% | "%scriptdir%/mtee.exe" /+ %log%
if %error% NEQ 0 echo.%shortfolder% is ALREADY a 7Zip and APPEARS CORRUPT ERROR code is %error% | "%scriptdir%/mtee.exe" /+ %log%
...
:CREATEERROR
echo.Datestamp is %date%, %time% ^& Current directory is %cd% >> %log%
echo.Couldn't create the 7zip from %shortfolder% | "%scriptdir%/mtee.exe" /+ %log%
echo.%shortfolder%.7z ARCHIVE CREATE ERROR code is %error% | "%scriptdir%/mtee.exe" /+ %log%
goto CONTINUE

:TESTERROR
echo.Datestamp is %date%, %time% ^& Current directory is %cd% >> %log%
echo.Archive test on %shortfolder% failed
echo.%shortfolder%.7z was just created but ARCHIVE TEST ERROR code is %error% | "%scriptdir%/mtee.exe" /+ %log%
goto CONTINUE
