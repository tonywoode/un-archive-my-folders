...
set folderdir=%~dp1
cd /d %~dp1
set log="%folderdir%CONVERT_MY_FOLDERS_zip_to_7zip%RANDOM%.log"
...
"C:\Program Files\7-Zip\7z.exe" a %zipfile%.7z %tempdir%\* 2<&1
...
echo.Made 7Z Archive %shortzip:~0,-5%".7z now testing archive... | "%scriptdir%/mtee.exe" /+ %log%
"C:\Program Files\7-Zip\7z.exe" t -r %zipfile%.7z >>%log% 2>&1
set error=%errorlevel%
if %error% EQU 0 echo.%shortzip:~0,-5%".7z CREATED AND TESTED - all OK | "%scriptdir%/mtee.exe" /+ %log%
...
echo.%shortzip% ------------------ERROR not a zip or doesn't exist | "%scriptdir%/mtee.exe" /+ %log%
...
:7ZEXISTS
echo.Datestamp is %date%, %time% ^& Current directory is %cd% >> %log%
echo.%shortzip:~0,-5%".7z already exists - checking... | "%scriptdir%/mtee.exe" /+ %log%
"C:\Program Files\7-Zip\7z.exe" t %zipfile%.7z >>%log% 2>&1
set error=%errorlevel%
if %error% EQU 0 echo.%shortzip:~0,-5%".7z ERROR ALREADY EXISTS and appears valid ERROR code is %error% :not processing | "%scriptdir%/mtee.exe" /+ %log%
if %error% NEQ 0 echo.%shortzip:~0,-5%".7z ERROR ALREADY EXISTS and APPEARS CORRUPT ERROR code is %error% : not processing| "%scriptdir%/mtee.exe" /+ %log%
goto CONTINUE
...
:CREATEERROR
echo.Datestamp is %date%, %time% ^& Current directory is %cd% >> %log%
echo.Either couldn't create the temp folder, or the 7zip, for %shortzip%, or %tempdir% (or its long-name equivalent) already exists so I don't want to mess with it| "%scriptdir%/mtee.exe" /+ %log%
echo.%shortzip:~0,-5%".7z ARCHIVE CREATE ERROR code is %error% | "%scriptdir%/mtee.exe" /+ %log%
goto CONTINUE
