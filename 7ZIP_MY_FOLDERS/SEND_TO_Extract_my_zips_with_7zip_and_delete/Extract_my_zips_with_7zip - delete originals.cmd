@echo off
SETLOCAL
echo.Extract_my_zips_with_7zip_and_delete.cmd


::	This script will unzip folder archives with 7zip, then delete the archives as it goes along
...
set folderdir=%~dp1
cd /d %~dp1
set log="%folderdir%Extract_my_zips_with_7zip_and_delete%RANDOM%.log"
...
"C:\Program Files\7-Zip\7z.exe" x %1 -o%dir%
set error=%errorlevel%
if %error% NEQ 0 goto CREATEERROR

echo.>>%log%
echo.>>%log%
echo.ARCHIVE CHECKED OK, AND 7ZIP REPORTED NO UNPACKING PROBLEMS - DELETING SOURCE ARCHIVE | "%scriptdir%/mtee.exe" /+ %log%
del %1 2>&1
set error=%errorlevel%
if %error% EQU 0 echo.%shortzip% DELETED from %folderdir% as unarchiving reports OK | "%scriptdir%/mtee.exe" /+ %log%
...
:NOTAZIP
if (%~x1)==(.7z) goto CARRYON
if (%~x1)==(.rar) goto CARRYON
echo.Datestamp is %date%, %time% ^& Current directory is %cd% >> %log%
echo.%shortzip% is not a zip, 7z, or rar file or doesn't exist | "%scriptdir%/mtee.exe" /+ %log%
echo.%shortzip% ------------------ERROR not an archive file or doesn't exist | "%scriptdir%/mtee.exe" /+ %log%
goto CONTINUE
...
:TESTZIP
echo.Datestamp is %date%, %time% ^& Current directory is %cd% >> %log%
echo.%shortzip% is an archive file - checking... | "%scriptdir%/mtee.exe" /+ %log%
